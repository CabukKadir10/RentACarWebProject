...
namespace RentACarProject.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("RentACarProject") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "RentACarProject", DisplayName = "RentACarProject API", Resources = { "RentACarProject" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["RentACarProject_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["RentACarProject_Web:RootUrl"]!.EnsureEndsWith('/');

            /* RentACarProject_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["RentACarProject_Web:ClientSecret"] ?? "1q2w3e*",
